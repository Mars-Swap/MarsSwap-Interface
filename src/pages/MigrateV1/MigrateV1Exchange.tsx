      ...
      <AutoRow style={{ justifyContent: 'flex-start', width: 'fit-content' }}>
        ...
        <div style={{ marginLeft: '.75rem' }}>
          <TYPE.mediumHeader>
            ...
            {chainId && token.equals(WETH[chainId]) ? 'WETH' : token.symbol}/Mars
          </TYPE.mediumHeader>
        </div>
      </AutoRow>
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        ...
        {chainId && (
          <ExternalLink href={getEtherscanLink(chainId, MIGRATOR_ADDRESS, 'address')}>
            <TYPE.blue display="inline">MarsSwap migration contract↗</TYPE.blue>
          </ExternalLink>
        )}
        .
      </TYPE.body>
      ...
      {!isFirstLiquidityProvider && largePriceDifference ? (
        <YellowCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            It{"'"}s best to deposit liquidity into MarsSwap V2 at a price you believe is correct. If the V2 price seems
          ...
          <AutoColumn gap="8px">
            <RowBetween>
              ...
              <TYPE.black>
                {v1SpotPrice?.toSignificant(6)} {token.symbol}/Mars
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v1SpotPrice?.invert()?.toSignificant(6)} Mars/{token.symbol}
              </TYPE.black>
            </RowBetween>
            ...
            <RowBetween>
              ...
              <TYPE.black>
                {v2SpotPrice?.toSignificant(6)} {token.symbol}/Mars
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v2SpotPrice?.invert()?.toSignificant(6)} Mars/{token.symbol}
              </TYPE.black>
            </RowBetween>
      ...
      {isFirstLiquidityProvider && (
        <PinkCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            You are the first liquidity provider for this pair on MarsSwap V2. Your liquidity will be migrated at the
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {`Your MarsSwap V1 ${token.symbol}/Mars liquidity will become MarsSwap V2 ${token.symbol}/Mars liquidity.`}
      </TYPE.darkGray>
    </AutoColumn>
  )
}
  ...
  const liquidityToken: Token | undefined = useMemo(
    () =>
      validatedAddress && chainId && token
        ? new Token(chainId, validatedAddress, 18, `UNI-V1-${token.symbol}`, 'MarsSwap V1')
        : undefined,
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          <div>
            <QuestionHelper text="Migrate your liquidity tokens from MarsSwap V1 to MarsSwap V2." />
          </div>
        </AutoRow>
        ...
        {!account ? (
          ...
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              Because MarsSwap V2 uses WETH under the hood, your MarsSwap V1 WETH/Mars liquidity cannot be migrated. You
