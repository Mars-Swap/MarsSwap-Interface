...
export function useUserHasLiquidityInAllTokens(): boolean | undefined {
  ...
  const v1ExchangeLiquidityTokens = useMemo(
    () =>
      chainId ? Object.keys(exchanges).map(address => new Token(chainId, address, 18, 'Mars-V1', 'MarsSwap V1')) : [],
    [chainId, exchanges]
  )
...
export function useV1Trade(
  ...
  // construct a direct or through Mars v1 route
  let pairs: Pair[] = []
  ...
  // if neither are Mars, it's token-to-token (if they both exist)
