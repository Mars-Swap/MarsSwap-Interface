...
const NETWORK_LABELS: { [chainId in ChainId]?: string } = {
  ...
  [ChainId.GÖRLI]: 'Görli',
  [ChainId.KOVAN]: 'Kovan',
  [ChainId.MarsTESTNET]: 'Mars',
  [ChainId.MarsMAINNET]: 'Mars'
}
  ...
  return (
    <HeaderFrame>
      ...
      <HeaderRow>
        <Title href=".">
          <UniIcon>
            <img width={'64px'} src={darkMode ? LogoDark : Logo} alt="logo" />
          </UniIcon>
        </Title>
        <HeaderLinks>
          ...
          <StyledNavLink id={`stake-nav-link`} to={'/Mars'}>
            POOL
          </StyledNavLink>
          {!account && (
            <div
              ...
              style={{ color: '#565A69', fontSize: '1rem', margin: '0 12px' }}
            >
              {t('Invitation')}
            </div>
          )}
            ...
            <UNIWrapper onClick={toggleClaimModal}>
              <UNIAmount active={!!account && !availableClaim} style={{ pointerEvents: 'auto' }}>
                <TYPE.white padding="0 2px">
                  {claimTxn && !claimTxn?.receipt ? <Dots>Claiming Mars</Dots> : 'Claim Mars'}
                </TYPE.white>
              </UNIAmount>
              <CardNoise />
            </UNIWrapper>
          ...
          <AccountElement active={!!account} style={{ pointerEvents: 'auto' }}>
            {account && userEthBalance ? (
              <BalanceText style={{ flexShrink: 0 }} pl="0.75rem" pr="0.5rem" fontWeight={500}>
                {userEthBalance?.toSignificant(4)} Mars
              </BalanceText>
            ) : null}
            <Web3Status />
          </AccountElement>
