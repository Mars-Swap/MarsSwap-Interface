...
const NETWORK_URL = process.env.REACT_APP_MarsSCAN || ''
...
export function getEtherscanLink(
  ...
  switch (type) {
    case 'transaction': {
      return `${prefix}?path=${encodeURIComponent(`/tx/${data}`)}`
    }
    case 'token': {
      return `${prefix}?path=${encodeURIComponent(`/token/${data}`)}`
    }
    case 'block': {
      return `${prefix}?path=${encodeURIComponent(`/block/${data}`)}`
    }
    case 'address':
    default: {
      return `${prefix}?path=${encodeURIComponent(`/address/${data}`)}`
    }
  }
}
